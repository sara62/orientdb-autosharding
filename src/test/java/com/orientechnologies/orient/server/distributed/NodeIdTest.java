...
public class NodeIdTest {
  ...
  public void testSubtractOverflow() {
    ...
    String result = two.toHexString();
    Assert.assertEquals(result, "ffffffffffffffffffffffffffffffffffffffffffffffff");
  }
  ...
  public void testToValueOfFromString() {
    final ONodeId nodeId = ONodeId.valueOf("00123456789abcdef0000000000123000000000000002aab");
  ...
  public void testRidToStream() {
    final OAutoShardedRecordId rid = new OAutoShardedRecordId(1, 124556237677935L, -65647577236563256L, 6709985366543467L);
    final byte[] ridStream = ONodeId.toStream(rid);
    final ONodeId nodeId = ONodeId.valueOf(rid);
    final byte[] nodeStream = nodeId.toStream();

    Assert.assertEquals(ridStream, nodeStream);
  }

  public void testConvertToRecordIdOne() {
    final OAutoShardedRecordId rid = new OAutoShardedRecordId(1, 124556237677935L, -65647577236563256L, 6709985366543467L);
    final ONodeId nodeId = ONodeId.valueOf(rid);

    final OAutoShardedRecordId convertedRid = ONodeId.convertToRecordId(nodeId, 1);

    Assert.assertEquals(convertedRid, rid);

  }

  public void testConvertToRecordIdTwo() {
    final ONodeId nodeId = ONodeId.valueOf("140000000000000000000000000000000000000000000000");

    final OAutoShardedRecordId convertedRid = ONodeId.convertToRecordId(nodeId, 1);

    final ONodeId convertedNodeId = ONodeId.valueOf(convertedRid);

    Assert.assertEquals(convertedNodeId, nodeId);
  }

  public void testCompareToRIDNodeIdCompatibility() {
    final TreeSet<ONodeId> nodeIds = new TreeSet<ONodeId>();
    final TreeSet<OAutoShardedRecordId> recordIds = new TreeSet<OAutoShardedRecordId>();

    for (int i = 0; i < 10000; i++) {
      final ONodeId nodeId = ONodeId.generateUniqueId();
      nodeIds.add(nodeId);

      recordIds.add(ONodeId.convertToRecordId(nodeId, 1));
    }

    final Iterator<OAutoShardedRecordId> recordIdIterator = recordIds.iterator();

    for (final ONodeId nodeId : nodeIds) {
      final OAutoShardedRecordId recordId = recordIdIterator.next();

      Assert.assertEquals(recordId, ONodeId.convertToRecordId(nodeId, 1));
    }
