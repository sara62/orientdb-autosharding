...
public class LocalMaintenanceProtocolImplTest {
  ...
  public void testPredecessorIsNull() {
    ...
    localMaintenanceProtocol.synchronizeReplicasBetweenHolders(localNode, STORAGE_NAME, CLUSTER_ID, 2, 1);
  ...
  public void testRootsAreLeavesAndEqual() {
    ...
    final ODetachedMerkleTreeNode localRootNode = new ODetachedMerkleTreeNode(hash,
        predecessorAddress.getNodeId().add(ONodeId.ONE), localNodeAddress.getNodeId(), CLUSTER_ID, recordMetadata, null, new int[0]);
    ...
    when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, localRootNode)).thenReturn(remoteRootNode);
    when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, localRootNode)).thenReturn(remoteRootNode);
  ...
  public void testVerifyOneChildInsideRootIntervalAndSecondIsNot() {
    ...
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.localRootNode, processFlow.remoteRootNode,
        processFlow.replicaHolderOne);

    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.localRootNode, processFlow.remoteRootNode,
        processFlow.replicaHolderTwo);

    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.firstLocalChildNode, processFlow.firstRemoteChildNode,
        processFlow.replicaHolderOne);
  ...
  public void testLeafChildIsNotEqualOnThirdLevel() {
    ...
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.firstLocalChildNodeSecondLevel,
        processFlow.firstRemoteChildNodeSecondLevel, processFlow.replicaHolderOne);
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.firstLocalChildNodeSecondLevel,
        processFlow.firstRemoteChildNodeSecondLevel, processFlow.replicaHolderTwo);

    verifyNoMoreInteractions(nodeComparator);
  }

  public void testPredecessorIsGreaterThanNodeIdSecondIntervalIsSameAsFirst() {
    PredecessorIsGreaterThanNodeIdSecondIntervalIsSameAsFirst processFlow = new PredecessorIsGreaterThanNodeIdSecondIntervalIsSameAsFirst();
    ...
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.secondLocalChildNode,
        processFlow.secondRemoteChildNode, processFlow.replicaHolderOne);
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.secondLocalChildNode,
        processFlow.secondRemoteChildNode, processFlow.replicaHolderTwo);

    verifyNoMoreInteractions(nodeComparator);
  }

  public void testPredecessorIsGreaterThanNodeIdSecondIntervalIsNotSameAsFirst() {
    PredecessorIsGreaterThanNodeIdSecondIntervalIsNotSameAsFirst processFlow = new PredecessorIsGreaterThanNodeIdSecondIntervalIsNotSameAsFirst();
    ...
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.secondLocalRootNode, processFlow.secondRemoteRootNode,
        processFlow.replicaHolderOne);
    verify(nodeComparator).compareNodes(STORAGE_NAME, localNode, processFlow.secondLocalRootNode, processFlow.secondRemoteRootNode,
        processFlow.replicaHolderTwo);
  ...
  public void testPredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameOne() {
    PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameOne processFlow = new PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameOne();
  ...
  public void testPredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameTwo() {
    PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameTwo processFlow = new PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameTwo();
  ...
  private class RemoteTreeNodeIsNull extends CreateTwoDifferentNodesWithOnlyOneInside {
    private RemoteTreeNodeIsNull() {
      when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, firstLocalChildNode)).thenReturn(null);
    }
  }
  ...
  private class PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameTwo extends
    ...
    private PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameTwo() {
      when(merkleTree.getRootNodesForInterval(ONodeId.ZERO, localNodeAddress().getNodeId())).thenReturn(
              Arrays.asList(localRootNode));
      when(merkleTree.getRootNodesForInterval(predecessorAddress().getNodeId().add(ONodeId.ONE), ONodeId.MAX_VALUE)).thenReturn(
              Arrays.asList(localRootNode, secondLocalRootNode));
    }
  }
  ...
  private class PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameOne extends
    ...
    private PredecessorIsGreaterThanNodeIdThreeRootNodesOneIsTheSameOne() {
      when(merkleTree.getRootNodesForInterval(ONodeId.ZERO, localNodeAddress().getNodeId())).thenReturn(
              Arrays.asList(localRootNode, secondLocalRootNode));
      when(merkleTree.getRootNodesForInterval(predecessorAddress().getNodeId().add(ONodeId.ONE), ONodeId.MAX_VALUE)).thenReturn(
              Arrays.asList(secondLocalRootNode));
    }
  }
  ...
  private class PredecessorIsGreaterThanNodeIdSecondIntervalIsNotSameAsFirst extends
    ...
    private PredecessorIsGreaterThanNodeIdSecondIntervalIsNotSameAsFirst() {
      ...
      secondLocalRootNode = new ODetachedMerkleTreeNode(secondRootLocalHash, predecessorAddress().getNodeId().add(ONodeId.ONE),
          ONodeId.MAX_VALUE, CLUSTER_ID, new ORecordMetadata[0], null, new int[0]);

      secondRemoteRootNode = new ODetachedMerkleTreeNode(secondRootRemoteHash, predecessorAddress().getNodeId().add(ONodeId.ONE),
          ONodeId.MAX_VALUE, CLUSTER_ID, new ORecordMetadata[0], null, new int[0]);
      ...
      when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, secondLocalRootNode)).thenReturn(secondRemoteRootNode);
      when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, secondLocalRootNode)).thenReturn(secondRemoteRootNode);
    }
  ...
  private class LeafChildIsNotEqualOnThirdLevel extends CreateTwoDifferentNodesWithOnlyOneInside {
    ...
    private LeafChildIsNotEqualOnThirdLevel() {
      ...
      localRootNode = new ODetachedMerkleTreeNode(localRootHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId(), CLUSTER_ID, null, localChildrenHash, new int[0]);

      remoteRootNode = new ODetachedMerkleTreeNode(remoteRootHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId(), CLUSTER_ID, null, remoteChildrenHash, new int[0]);

      final ODetachedMerkleTreeNode localChildNode = new ODetachedMerkleTreeNode(childHash, predecessorAddress.getNodeId().add(
          ONodeId.ONE), localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID, recordMetadata, null, new int[0]);
      ...
      firstLocalChildNode = new ODetachedMerkleTreeNode(firstLocalDiffChildHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID, null, localChildrenHashSecondLevel, new int[0]);

      firstRemoteChildNode = new ODetachedMerkleTreeNode(firstRemoteDiffChildHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID, null, remoteChildrenHashSecondLevel, new int[0]);

      final ODetachedMerkleTreeNode secondLocalChildNode = new ODetachedMerkleTreeNode(secondLocalDiffChildHash, localNodeAddress
          .getNodeId().add(ONodeId.ONE), localNodeAddress.getNodeId().add(ONodeId.TWO), CLUSTER_ID, recordMetadata, null,
          new int[0]);
      ...
      firstLocalChildNodeSecondLevel = new ODetachedMerkleTreeNode(childHashDiffLocalThirdLevel, predecessorAddress.getNodeId()
          .add(ONodeId.ONE), localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID, recordMetadata, null, new int[0]);

      firstRemoteChildNodeSecondLevel = new ODetachedMerkleTreeNode(childHashDiffRemoteThirdLevel, predecessorAddress.getNodeId()
          .add(ONodeId.ONE), localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID, recordMetadata, null, new int[0]);

      final ODetachedMerkleTreeNode localChildNodeSecondLevel = new ODetachedMerkleTreeNode(childHashThirdLevel, predecessorAddress
          .getNodeId().add(ONodeId.ONE), localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID, recordMetadata, null,
          new int[0]);

      final ODetachedMerkleTreeNode remoteChildNodeSecondLevel = new ODetachedMerkleTreeNode(childHashThirdLevel,
          predecessorAddress.getNodeId().add(ONodeId.ONE), localNodeAddress.getNodeId().subtract(ONodeId.TWO), CLUSTER_ID,
          recordMetadata, null, new int[0]);
      ...
      for (int i = 0; i < 64; i++) {
        if (i == 2) {
          ...
          when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, firstLocalChildNode)).thenReturn(firstRemoteChildNode);
          when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, firstLocalChildNode)).thenReturn(firstRemoteChildNode);
        } else if (i == 63) {
          when(merkleTree.getChildNode(localRootNode, i)).thenReturn(secondLocalChildNode);
          when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, secondLocalChildNode)).thenReturn(secondRemoteChildNode);
          when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, secondLocalChildNode)).thenReturn(secondRemoteChildNode);
        } else {
          when(merkleTree.getChildNode(localRootNode, i)).thenReturn(localChildNode);
          when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, localChildNode)).thenReturn(remoteChildNode);
          when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, localChildNode)).thenReturn(remoteChildNode);
        }
      }
      ...
      for (int i = 0; i < 64; i++) {
        if (i == 0) {
          ...
          when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, firstLocalChildNodeSecondLevel)).thenReturn(firstRemoteChildNodeSecondLevel);
          when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, firstLocalChildNodeSecondLevel)).thenReturn(firstRemoteChildNodeSecondLevel);
        } else {
          when(merkleTree.getChildNode(firstLocalChildNode, i)).thenReturn(localChildNodeSecondLevel);
          when(remoteNodeOne.findMerkleTreeNode(STORAGE_NAME, localChildNodeSecondLevel)).thenReturn(remoteChildNodeSecondLevel);
          when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, localChildNodeSecondLevel)).thenReturn(remoteChildNodeSecondLevel);
        }
      }
  ...
  private class RootNodesAreDifferentLocalIsLeafButRemoteIsNot extends CreateTwoDifferentNodesWithOnlyOneInside {
    private RootNodesAreDifferentLocalIsLeafButRemoteIsNot() {
      ...
      remoteRootNode = new ODetachedMerkleTreeNode(localRootHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId(), CLUSTER_ID, null, remoteChildHash, new int[0]);

      localRootNode = new ODetachedMerkleTreeNode(remoteRootHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId(), CLUSTER_ID, recordMetadata, null, new int[0]);
  ...
  private class RootNodesAreDifferentRemoteIsLeafButLocalIsNot extends CreateTwoDifferentNodesWithOnlyOneInside {
    private RootNodesAreDifferentRemoteIsLeafButLocalIsNot() {
      ...
      localRootNode = new ODetachedMerkleTreeNode(localRootHash, predecessorAddress.getNodeId().add(ONodeId.ONE),
          localNodeAddress.getNodeId(), CLUSTER_ID, null, localChildHash, new int[0]);
  ...
  private class OneReplicaHolderThrowsExceptionDuringProcessing extends CreateTwoDifferentNodesWithOnlyOneInside {
    private OneReplicaHolderThrowsExceptionDuringProcessing() {
      when(remoteNodeTwo.findMerkleTreeNode(STORAGE_NAME, firstLocalChildNode)).thenThrow(new RuntimeException("Fake exception"));
    }
  }
  ...
  private class CreateTwoDifferentNodesWithOnlyOneInside {
    ...
    private CreateTwoDifferentNodesWithOnlyOneInside() {
      ...
      localRootNode = new ODetachedMerkleTreeNode(localRootHash, localRootNodeStartId(), localRootNodeEndId(), CLUSTER_ID, null,
          localChildHash, new int[0]);

      remoteRootNode = new ODetachedMerkleTreeNode(remoteRootHash, remoteRootNodeStartId(), remoteRootNodeEndId(), CLUSTER_ID,
          null, remoteChildHash, new int[0]);

      final ODetachedMerkleTreeNode localChildNode = new ODetachedMerkleTreeNode(childHash, localChildNodeStartId(),
          localChildNodeEndId(), CLUSTER_ID, recordMetadata, null, new int[0]);

      final ODetachedMerkleTreeNode remoteChildNode = new ODetachedMerkleTreeNode(childHash, remoteChildNodeStartId(),
          remoteChildNodeEndId(), CLUSTER_ID, recordMetadata, null, new int[0]);

      firstLocalChildNode = new ODetachedMerkleTreeNode(firstLocalDiffChildHash, firstLocalChildNodeStartId(),
          firstLocalChildNodeEndId(), CLUSTER_ID, recordMetadata, null, new int[0]);

      firstRemoteChildNode = new ODetachedMerkleTreeNode(firstRemoteDiffChildHash, firstRemoteChildNodeStartId(),
          firstRemoteChildNodeEndId(), CLUSTER_ID, recordMetadata, null, new int[0]);

      secondLocalChildNode = new ODetachedMerkleTreeNode(secondLocalDiffChildHash, secondLocalChildNodeStartId(),
          secondLocalChildNodeEndId(), CLUSTER_ID, recordMetadata, null, new int[0]);

      secondRemoteChildNode = new ODetachedMerkleTreeNode(secondRemoteDiffChildHash, secondRemoteChildNodeStartId(),
          secondRemoteChildNodeEndId(), CLUSTER_ID, recordMetadata, null, new int[0]);

      when(localNode.getLocalMerkleTree(STORAGE_NAME, CLUSTER_ID)).thenReturn(merkleTree);
      when(merkleTree.getRootNodesForInterval(localChildNodeStartId(), localRootNodeEndId())).thenReturn(
              Arrays.asList(localRootNode));
